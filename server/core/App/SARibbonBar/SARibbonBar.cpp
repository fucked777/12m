...
public:
    ...
    SARibbonBarPrivate(SARibbonBar* par)
        ...
        ,unusableTitleRegion(0)  //FIXME 高度
        ,meanBarHeight(50)        //FIXME 高度
    ...
    if(category)
    {
        ...
        if (isRibbonBarHideMode())
        {
            if(!m_d->stackedContainerWidget->isVisible())
            {
                ...
                {
                    //FIXME
                    m_d->stackedContainerWidget->setFixedHeight(m_d->meanBarHeight - m_d->tabBarHight);
...
void SARibbonBar::updateRibbonElementGeometry(SARibbonBar::RibbonStyle style)
{
    ...
    {
    case OfficeStyle:
        m_d->meanBarHeight = 112;
        break;
    case WpsLiteStyle:
        m_d->meanBarHeight = 90;//FIXME
        break;
    default:
        m_d->meanBarHeight = 80;
        break;
    }
...
//                          ,m_d->titleBarHight);
        QRect titleRegion(0
                          ,m_d->widgetBord.top()
                          ,m_d->ribbonTabBar->geometry().right()-0
                          ,m_d->titleBarHight);
        paintWindowTitle(p,parWindow->windowTitle(),titleRegion);
        paintWindowIcon(p,parWindow->windowIcon());
    }
