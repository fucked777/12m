...
namespace
{
    ...
    public:
        ...
        {
            auto server = std::make_shared<DistanceCalibration>(context);
            ServiceProperties props;
            props["Type"] = std::string("DistanceCalibration");
            context.RegisterService<DistanceCalibration>(server, props);

            server->setConnectAddr(ZMQHelper::deaultLocalAddress());
            server->setSrcAddr("DistanceCalibration");
            QString msg;
