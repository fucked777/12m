...
REGISTER_PRIVAE_CLASS_TEMPLATE(RMRETTest, KaKuo2, createTestItem);
...
bool RMRETTest::linkConfiguration()
{
    /*
     * 20210827 wp?
     * 已经询问过闫总  颜总
...
bool RMRETTest::parameterSetting()
{
...
bool RMRETTest::testing(CmdResult& cmdResult)
{
    PackCommand packCommand;
...
bool RMRETTest::resultOperation(const CmdResult& cmdResult)
{
    if (!cmdResult.m_paramdataMap.contains("RangMean"))
    {
        pushLog("测试错误:当前上报结果 测距均值 缺失", true);
        return false;
    }
    if (!cmdResult.m_paramdataMap.contains("RandDiff"))
    {
        pushLog("测试错误:当前上报结果 测距随机差 缺失", true);
        return false;
    }
