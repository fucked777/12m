...
public:
    ...
    /* 前端测试信号选择开关切换 这里是本振信号 */
    static bool frontLocalVibrationEndSwitchChange(SingleCommandHelper& singleCmdHelper, AutomateTestBase* self);
    /* 本振输出选择 */
    static bool localVibrationTestSignChoose(SingleCommandHelper& singleCmdHelper, AutomateTestBase* self);
    /* 前端信号源接入场放 */
    static bool frontSignalSourceToLNA(SingleCommandHelper& singleCmdHelper, AutomateTestBase* self);
    ...
    /* 发射分系统的链路
     * 设备太少了 现有的链路怎打都会有额外不使用的设备
     */
    static bool sendSystemLink(SingleCommandHelper& singleCmdHelper, DeviceID& hp, AutomateTestBase* self);
};
