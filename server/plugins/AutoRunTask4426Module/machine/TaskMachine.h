...
public:
    ...
    void stopRing();
    // 设置和获取任务计划
    TaskPlanData taskPlan() const;
    void setTaskPlan(const TaskPlanData& taskPlanData);
    QString curTaskDevUuid() const;
    ...
    /* 还原任务 */
    void restoreTaskRuning(TaskStep taskStep, const QString& uuid, const QMap<TaskStep, TaskStepStatus>&);
...
private:
    ...
    std::atomic<bool> mIsRunning{ true };               // 标识线程是否运行
    ...
    QThread m_workThread;                               // 动态工作者线程
    TaskPlanData mTaskPlanData;                         // 任务计划的完整数据
    QString m_machineUUID;
    BaseEvent* m_currentEvent = nullptr;

    int m_restoreSleepTime=0;
