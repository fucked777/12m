...
Optional<PhaseCalibrationItem> SqlPhaseCalibration::insertItem(const PhaseCalibrationItem& bak)
{
    ...
    AutoCloseDBHelper closeHelper;
    {
        ...
        sqlQuery.bindValue(4, Utility::dateTimeToStr(item.createTime));
        sqlQuery.bindValue(5, static_cast<int>(item.workMode));
        ...
        sqlQuery.bindValue(15, array, QSql::ParamTypeFlag::Binary);

        auto tempKey = QString("%1_%2_%3_%4")                     //
                           .arg(item.taskCode)                    //
                           .arg(static_cast<int>(item.workMode))  //
                           .arg(item.dotDrequency)
                           .arg(item.equipComb);

        sqlQuery.bindValue(16, tempKey);
...
static QString createCondQuerySqlPhaseCalibration(const QueryPhaseCalibrationHistry& cond)
{
    ...
    if (cond.workMode == SystemWorkMode::NotDefine)
    {
    ...
    QString sqlTemplate = "select * from "
                          "( %1 ) as temptable "
                          "where taskCode = '%2' and  workMode = '%3'; ";
    return sqlTemplate.arg(sql, cond.taskCode).arg(cond.workMode);
}
...
Optional<PhaseCalibrationItemList> SqlPhaseCalibration::selectItem(const QString& taskCode, int pointFreqNo, SystemWorkMode workMode,
                                                                   quint64 equipComb)
{
    using ResType = Optional<PhaseCalibrationItemList>;
    AutoCloseDBHelper closeHelper;
    {
        ...
        QString sqlTemplate("SELECT * FROM PhaseCalibration where selectCondKey='%1_%2_%3_%4' order by createTime desc limit 10;");
        sqlTemplate = sqlTemplate                           //
                          .arg(taskCode)                    //
                          .arg(static_cast<int>(workMode))  //
                          .arg(pointFreqNo)
                          .arg(equipComb);
        QSqlQuery sqlQuery = db->exec(sqlTemplate);

        auto sqlErr = sqlQuery.lastError();
        if (sqlErr.isValid())
        {
            return ResType(ErrorCode::SqlExecFailed, sqlErr.text());
        }
