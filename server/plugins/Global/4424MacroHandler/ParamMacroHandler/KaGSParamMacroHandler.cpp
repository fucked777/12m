...
bool KaGSParamMacroHandler::handle()
{
    ...
    // 设置Ka频段射频开关网络参数
    DeviceID kapdspkgwlDeviceID(3, 6, 1);
    QMap<int, QMap<QString, QVariant>> kakgwlUnitParamMap;
    paramMacroModeData.getDeviceUnitParamMap(kapdspkgwlDeviceID, kakgwlUnitParamMap);
    auto kakgwlParamMap = kakgwlUnitParamMap[1];
    singleCommandHelper.packSingleCommand(QString("StepKAPDSPKGWL_XHSJ"), packCommand, kapdspkgwlDeviceID, kakgwlParamMap);
    waitExecSuccess(packCommand);

    // 设置Ka跟踪下变频器A、B参数
    DeviceID kagzxbpqADeviceID(3, 7, 1);
    QMap<int, QMap<QString, QVariant>> kagzxbpqUnitParamMap;
    paramMacroModeData.getDeviceUnitParamMap(kagzxbpqADeviceID, kagzxbpqUnitParamMap);
    auto kagzxbpqParamMap = kagzxbpqUnitParamMap[1];
    kagzxbpqParamMap["InputFreq"] = downFrequency;  // 替换为点频的下行频率

    singleCommandHelper.packSingleCommand(QString("StepKAGZXBPQ_SJL"), packCommand, kagzxbpqADeviceID, kagzxbpqParamMap);
    waitExecSuccess(packCommand);

    DeviceID kagzxbpqBDeviceID(3, 7, 2);
    singleCommandHelper.packSingleCommand(QString("StepKAGZXBPQ_SJL"), packCommand, kagzxbpqBDeviceID, kagzxbpqParamMap);
    waitExecSuccess(packCommand);

    // 设置Ka1.2GHz下变频器参数
    DeviceID kaxbpqDeviceID;
    if (!getKaXBPQDeviceID(kaxbpqDeviceID))
    {
        return false;
    }
    QMap<int, QMap<QString, QVariant>> kaxbpqUnitParamMap;
    paramMacroModeData.getDeviceUnitParamMap(kagzxbpqADeviceID, kaxbpqUnitParamMap);
    auto kaxbpqParamMap = kaxbpqUnitParamMap[1];
    kaxbpqParamMap["InputFreq1"] = downFrequency;  // 替换为点频的下行频率
    kaxbpqParamMap["InputFreq2"] = downFrequency;
    singleCommandHelper.packSingleCommand(QString("StepKAXBPQ_SJL"), packCommand, kaxbpqDeviceID, kaxbpqParamMap);
    waitExecSuccess(packCommand);
...
bool KaGSParamMacroHandler::getGSJDUnitTargetParamMap(const TargetInfo& targetInfo, QMap<int, QMap<int, QMap<QString, QVariant>>>& unitTargetParamMap)
{
    auto workModeDesc = SystemWorkModeHelper::systemWorkModeToDesc(mLinkLine.workMode);
