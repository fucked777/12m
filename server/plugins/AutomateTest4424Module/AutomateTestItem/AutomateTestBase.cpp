...
AutomateTestBase::~AutomateTestBase() { delete m_impl; }
...
void AutomateTestBase::run()
{
     ...
     */
    {
        ...
        m_upFreq = up.toDouble() * 1000;  //因为测试计算机命令里的单位都是kHz，而获取到的上下行频率单位是MHz，故需要先乘以1000
        m_downFreq = down.toDouble() * 1000;
...
void AutomateTestBase::stopImpl() {}
                                                                         ...
                                                                         qint32 ttl, const QVariant& arg, ConditionFunc func)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        while (timeout > 0)
        {
            ...
            {
                pushLog(QString("结果数据不正确 : %1").arg(currentCmdOperatorInfo), true);
                return ExecStatus::Failed;
            }
...
void AutomateTestBase::notWaitExecSuccessImpl(const PackCommand& packCommand, bool isError)
{
    if (!packCommand.errorMsg.isEmpty())
    {
        pushLog(packCommand.errorMsg, isError);
        return;
    }

    if (packCommand.data.isEmpty())
    {
        pushLog("系统组包失败，请检查配置", isError);
        return;
    }
    m_impl->isRecvResponce = false;
...
void AutomateTestBase::notWaitExecSuccess(const PackCommand& packCommand) { notWaitExecSuccessImpl(packCommand, true); }
void AutomateTestBase::notWaitExecSuccessBeQuiet(const PackCommand& packCommand) { notWaitExecSuccessImpl(packCommand, false); }
