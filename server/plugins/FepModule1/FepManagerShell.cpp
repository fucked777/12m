...
void FepManagerShell::init()
{
    ...
    auto path = PlatformConfigTools::configBusiness(setting.value("FepConfig/path", "FepRecv").toString());
    path = QDir(path).absolutePath();
    m_fepServer->setWorkDir(path);
...
void FepManagerShell::getPlan(QString workDir)
{
    ...
    m_fepSend->setWorkDir(workDir);
    QString errMsg;
    if (!m_fepSend->start(errMsg))
    {
        // QMessageBox::information(this, "发送错误", errMsg);
        return;
    }
