...
void NumberStatusPowerLabel::setValue(const QVariant& value)
{
    // 去掉小数后多于的0
    QRegExp rx(R"((\.){0,1}0+$)");
    //    auto valuedBmString = QString("%1").arg(value.toDouble(), 0, 'f').replace(rx, "");
    //获取转换成w后的值
    auto valueWString = QString::number(WdBmConversion(value.toDouble(), false));
    setText(valueWString);
    setStatus(Normal);
}
    ...
    while (!domNode.isNull())
    {
        ...
        while (!subChildNode.isNull())
        {
            ...
            if (subChildEle.tagName() == "MainInfo")
            {
                ...
                sumInfo.mainInfoList.append(mainInfo);
            }
