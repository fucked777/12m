#ifndef LEDSTATUSSTIMULABEL_H
#define LEDSTATUSSTIMULABEL_H
...
/**
 * @brief The LEDStatusStimuLabel class
 * 控件说明：
 * 这个控件主要解决的问题是动态显示26测控信道的激励状态
 * 显示逻辑:
 * 1.测控信道分别S测控、Ka测控和Ka低速
 * 2.S测控激励从S功放切换开关取出参数ID为"SwitcherSet"的参数值,判断出当前在线的功放是哪个
 * 然后再到对应的功放中取参数ID为"ElectricControlor"的参数值,作为激励状态
 * 3.Ka测控则是从Ka测控基带就可以取出在线状态和激励状态
 * 4.Ka低速同Ka测控
 *
 * 刷新逻辑:
 * 1.在MainScreenControlFactory类中添加类型名为LEDStatusStimuLabel的控件
 ...
 * 5.把配置文件中的刷新信息存入成员变量QMap<QString, QList<LEDStatusStimuLabel::SumInfo>> d_stimuInfos中
 * 6.按照显示逻辑刷新该控件
 *
 * 属性介绍
...
class LEDStatusStimuLabel : public BaseWidget, public StatusController
{
    Q_OBJECT
...
public:
    ...
    {
        ParamInfo paramInfo;
        QList<MainInfo> mainInfoList;
        QMap<QString, QString> ledInfo;
    };

    explicit LEDStatusStimuLabel(const QDomElement& domEle, QWidget* parent = nullptr);
...
private:
    ...
    QMap<QString, LEDItem> mLedItemMap;
