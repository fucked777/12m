...
void SCTaskWorkModeTaskCodeSelectWidget::refreshRunningTaskPlanData(const ManualMessage& msg)
{
    ...
    //有任务的时候才进行刷新处理，不然会把手动界面当前记录的值全部刷新回去
    if (!manualMsg.linkLineMap.isEmpty())
    {
        //Ka低速
        if (mdefaultSystemWorkMode == KaDS && !manualMsg.linkLineMap.contains(KaDS))
        {
            ui->scTargetRadioBtn1->setChecked(true);
            ui->scTaskCodeCombo1->setCurrentIndex(0);
            ui->scTaskCodeCombo2->setCurrentIndex(0);
        }
        //X低速
        if (mdefaultSystemWorkMode == XDS && !manualMsg.linkLineMap.contains(XDS))
        {
            ui->scTargetRadioBtn1->setChecked(true);
            ui->scTaskCodeCombo1->setCurrentIndex(0);
            ui->scTaskCodeCombo2->setCurrentIndex(0);
        ...
        //Ka高速
        if (mdefaultSystemWorkMode == KaGS && !manualMsg.linkLineMap.contains(KaGS))
        {
            ui->scTargetRadioBtn1->setChecked(true);
            ui->scTaskCodeCombo1->setCurrentIndex(0);
            ui->scTaskCodeCombo2->setCurrentIndex(0);
