...
void CKTaskWorkModeTaskCodeSelectWidget::refreshRunningTaskPlanData(const ManualMessage& msg)
{
    ...
    {
        //S测控
        if (m_workModeType == 0 && (!manualMsg.linkLineMap.contains(STTC) && !manualMsg.linkLineMap.contains(Skuo2) && !manualMsg.linkLineMap.contains(SKT)))
        {
            ui->ckTargetRadioBtn1->setChecked(true);
            ui->ckWorkModeCombo->setCurrentIndex(0);
        }
        //Ka测控
        if (m_workModeType != 0 && !manualMsg.linkLineMap.contains(KaKuo2))
        {
            ui->ckTargetRadioBtn1->setChecked(true);
            ui->ckWorkModeCombo->setCurrentIndex(0);
        }
