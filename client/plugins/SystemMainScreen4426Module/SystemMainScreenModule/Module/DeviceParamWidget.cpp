...
//            }
            ...
            auto& mainInfoList = info.mainInfoList;
            QList<bool> mainDataTrueList;
            //只有S功放会这样处理，因为它的激励灯不取决于某个值，而是三个值一起来判断的，和Ka功放的不一样
            auto paramInfo = info.paramInfo;
            if ((paramInfo.deviceId.sysID == 2 && paramInfo.deviceId.devID == 0 && paramInfo.deviceId.extenID == 1) ||
                (paramInfo.deviceId.sysID == 2 && paramInfo.deviceId.devID == 0 && paramInfo.deviceId.extenID == 2) ||
                (paramInfo.deviceId.sysID == 2 && paramInfo.deviceId.devID == 1 && paramInfo.deviceId.extenID == 1))
            {
                for (auto& mainInfo : mainInfoList)
                {
                    QString currentOnlineSta = dynamicDataMap[mainInfo.deviceId].unitReportMsgMap[mainInfo.unitId].paramMap[mainInfo.paramId].toString();
                ...
                //只有某一个功放加激励的时候才break
                if (!mainDataTrueList.isEmpty() && mainDataTrueList.size() == mainInfoList.size())
                {
                    QString sValue = info.ledInfo[QString::number(1)];
                    d_currentComplextDataMap[uniqueKey] = sValue;//加激励
                    break;
                }
                else if(!mainDataTrueList.isEmpty())
                {
                    QString sValue = info.ledInfo[QString::number(2)];
                    d_currentComplextDataMap[uniqueKey] = sValue;//去激励
                }
                else
                {
                    QString sValue = info.ledInfo[QString::number(0)];
                    d_currentComplextDataMap[uniqueKey] = sValue;
                }
            }
            else
            {
                ...
                if (mainDataTrueList.size() == mainInfoList.size())  // mainInfoList里面的值必须都相等
                {
                    ...
                    QString sValue = info.ledInfo[paramValue.toString()];
                    d_currentComplextDataMap[uniqueKey] = sValue;
                    break;
                }
            }
