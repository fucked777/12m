...
TimeSyncUDPGB::~TimeSyncUDPGB()
{
    ...
    {
        m_impl->socket->close();
        m_impl->socket->waitForDisconnected();
...
OptionalNotValue TimeSyncUDPGB::initSocket()
{
    ...
    //    }
    connect(socket, &QIODevice::readyRead, this, &TimeSyncUDPGB::slotReadyRead);
    m_impl->socket = helper.get();
    return OptionalNotValue();
}

void TimeSyncUDPGB::slotReadyRead()
{
