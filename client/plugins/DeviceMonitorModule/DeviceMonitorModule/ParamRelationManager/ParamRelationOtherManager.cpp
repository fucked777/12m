...
{
    int codeWay = setParamData.value("CodeWay").toInt();            //译码方式

    int viterbiCode = setParamData.value("ViterbiDecoding").toInt();  // Viterbi译码

    int rsDecoding = setParamData.value("RSDecoding").toInt();  // R-S译码

    int ldpcType = setParamData.value("LDPCDecoding").toInt();  // Ldpc译码

    int turboType = setParamData.value("TurboDecoding").toInt();  // Turbo译码

    if (codeWay == 2 && viterbiCode == 0)
    {
        errorMsg = QString("当译码方式为Viterbi译码时，Viterbi译码选择不能为否");
        return false;
    }
    else if (codeWay == 3 && rsDecoding == 0)
    {
        errorMsg = QString("当译码方式为R-S译码时，R-S译码选择不能为否");
        return false;
    }
    else if (codeWay == 4 && (viterbiCode == 0 || rsDecoding == 0))
    {
        errorMsg = QString("当译码方式为R-S译码和Viterbi级联译码时，R-S译码和Viterbi译码选择不能为否");
        return false;
    }
    else if (codeWay == 5 && ldpcType == 0)
    {
        errorMsg = QString("当译码方式为LDPC译码时，LDPC译码选择不能为无");
        return false;
    }
    else if (codeWay == 6 && turboType == 0)
    {
        errorMsg = QString("当译码方式为Turbo译码时，Turbo译码选择不能为无");
        return false;
    }
