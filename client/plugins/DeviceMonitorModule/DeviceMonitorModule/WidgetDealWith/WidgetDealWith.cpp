...
{
    if (m_currentDeviceID.sysID == 4 && m_currentDeviceID.devID == 4 && (unitID == 8 || unitID == 9) &&
                                               ...
                                               const QMap<QString, QVariant>& setParamJTMap)
{
    ...
    int tztz = setParamJSMap.value("CarrierModulation").toInt();        //调制体制
    int codeWay = setParamJTMap.value("CodeWay").toInt();            //码型
    int codeType = setParamJTMap.value("CodeType").toInt();            //码型
    int viterbiCode = setParamJTMap.value("ViterbiDecoding").toInt();  // Viterbi译码

    int ldpcType = setParamJTMap.value("LDPCDecoding").toInt();  // Ldpc译码
    int ldpcCode = setParamJTMap.value("LDPCCodeRate").toInt();  // Ldpc码率

    int turboType = setParamJTMap.value("TurboDecoding").toInt();  // Turbo译码
    int turboCode = setParamJTMap.value("TurboCodeRate").toInt();  // Turbo码率
    ...
    if (viterbiCode != 0 && (codeWay == 2 || codeWay == 4))  // Viterbi不为否且译码开关为Viterbi或者RS+Viterbi的时候
    {
        if (viterbiCode == 1)  //(7,(1/2))
        {
            cValue = QString("1/2");
    ...
    else if (turboType == 1 && codeWay == 6)  // Turbo译码为有
    {
        if (turboCode == 0)  // 1/2
        {
            cValue = QString("1/2");
    ...
    else if (ldpcType == 1 && codeWay == 5)  // LDPC译码为有
    {
        if (ldpcCode == 0)  // 1/2
        {
            cValue = QString("1/2");
