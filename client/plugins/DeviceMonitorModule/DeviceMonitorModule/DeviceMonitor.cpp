            ...
            if (attr.isShow)
            {
                ...
                if (control)
                {
                    ...
                    connect(control, static_cast<void (WWidget::*)(const QVariant&)>(&WWidget::sig_valueChange), this, [=](const QVariant& value) {
                        ...
                        else if (m_currentDeviceID.sysID == 4 && m_currentDeviceID.devID == 2 && (m_currentMode.ID == 10 || m_currentMode.ID == 11 || m_currentMode.ID == 12))
                        {
...
bool DeviceMonitor::checkParamRelation(QMap<QString, QVariant>& setParamData, QString& errorMsg)
{
    ...
    else if (systemId == 4 && deviceId == 2 && (unitID == 5 || unitID == 6) && (modeId == 10 || modeId == 11 || modeId == 12))
    {
...
void DeviceMonitor::dealWithGZMSLNumRange(int unitID, const QString& objName)
{
    if (objName == "CarrierModulation" || objName == "CodeType" || objName == "ViterbiDecoding" || objName == "LDPCDecoding" ||
        objName == "LDPCCodeRate" || objName == "TurboDecoding" || objName == "TurboCodeRate" || objName == "CodeWay")
    {
        if (unitID == 4)
        {
            WWidget* codeSpeedI = nullptr;
            WWidget* codeSpeedQ = nullptr;
            if (m_currentMode.ID == 12)
            {
                codeSpeedI = getDeviceWidgetParamControl(3, "CodeSpeed");//这里的3是指的index 不是单元号
                codeSpeedQ = getDeviceWidgetParamControl(4, "CodeSpeed");//这里的4是指的index 不是单元号
            }
            else
            {
                codeSpeedI = getDeviceWidgetParamControl(4, "CodeSpeed");//这里的4是指的index 不是单元号
                codeSpeedQ = getDeviceWidgetParamControl(5, "CodeSpeed");//这里的5是指的index 不是单元号
            }
