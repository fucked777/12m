...
public:
    ...
    static QByteArray createPack(quint8 paUI, quint8 jgUI, quint16 taskCode);
...
QByteArray StationNetCenterCmdTestImpl::createPack(quint8 paUI, quint8 jgUI, quint16 taskCode)
{
    PDXPHeader sendHeader;
    sendHeader.version = 0x01;
    sendHeader.mid = taskCode;
    sendHeader.sid = 0xFEDCBA;
    sendHeader.did = 0x66043208;
    sendHeader.bid = ZWZX_YCKZML_TYPE;
    sendHeader.no = 0;
    sendHeader.flag = 0;
    sendHeader.reserve = 0;
    sendHeader.date = getAccumDate();
    sendHeader.time = GlobalData::currentTime().msecsSinceStartOfDay() * 10;

    auto msg = PackPDXPHeader::packHead(sendHeader);
    quint8 v{ 0x02 };   // 版本号
    quint8 st{ 0x01 };  // 执行方式
    quint8 pa = paUI;   // 状态参数
    quint8 jg = jgUI;   // 执行结果
    quint32 res{ 0 };   // 保留
    sendHeader.len = static_cast<quint16>(sizeof(v) + sizeof(st) + sizeof(pa) + sizeof(jg) + sizeof(res));
    msg.append(CConverter::toByteArray<quint16>(sendHeader.len));
...
StationNetCenterCmdTest::StationNetCenterCmdTest(cppmicroservices::BundleContext /*context*/, QWidget* parent)
    : QWidget(parent)
...
void StationNetCenterCmdTest::send()
{
    ...
    auto sendMsg = StationNetCenterCmdTestImpl::createPack(ui->pa->value(), ui->jg->value(), ui->mid->text().toUInt(nullptr, 16));
    /* 连续发3次 */
    bool sendRet = true;
...
bool StationNetCenterCmdTest::parseData(const QByteArray& data)
{
    log(data.toHex());
    /* 解包 */
    auto result = PackPDXPHeader::unpackHead(data);
    if (!result)
    {
        log("解包失败");
        return false;
