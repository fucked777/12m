...
void ZMQLocalRouterServer::run()
{
    ...
    //        return;
    //    }
    auto socketInit = SocketPackHelper::initRouterSocket(*zmqContext, localSocket, ZMQHelper::deaultLocalAddress(), "localSocket");
    if (!socketInit)
    {
    ...
    /* 初始化轮询集合 */
    zmq_pollitem_t items[2] = { { localSocket.handle(), 0, ZMQ_POLLIN, 0 },
                                { nullptr, 0, ZMQ_POLLIN, 0 } };  //, { externalRecvSocket.handle(), 0, ZMQ_POLLIN, 0 } };

    while (m_impl->runningFlag)
    {
        /* 主备 */
        auto masterSlaveInfo = GlobalData::masterSlaveInfo();
        if(masterSlaveInfo.currentMasterIP.isEmpty())
        {
            QThread::sleep(1);
            continue;
        }
        if (m_impl->masterIP != masterSlaveInfo.currentMasterIP)
        {
            m_impl->masterIP = masterSlaveInfo.currentMasterIP;
            auto communicationPort = ExtendedConfig::clientCommunicationPort();
            m_impl->convertAddr = ZMQHelper::syntheticAddress("tcp", masterSlaveInfo.currentMasterIP, communicationPort);
            /* 重置网络 */
            externalRecvSocket.destroy();
            externalSendSocket.destroy();
            auto socketInit = SocketPackHelper::initDealerSocket(*zmqContext, externalRecvSocket, m_impl->convertAddr, m_impl->convertRecvID);
            if (!socketInit)
            {
                qWarning() << "ZMQLocalRouterServer ExternalRecvSocket init error:" << socketInit.msg();
            ...
            items[1] = { externalRecvSocket.handle(), 0, ZMQ_POLLIN, 0 };
        }
