#ifndef ParamMacroRelationManager_H
#define ParamMacroRelationManager_H
...
class ParamMacroRelationManager : public QObject
{
    Q_OBJECT
public:
    explicit ParamMacroRelationManager(QObject* parent = nullptr);

    void addUnit(SystemWorkMode workMode, QMap<QString, QMap<QString, WWidget*>> deviceMap);
...
private:
    ...
    void initUnitRelationControl(QMap<QString, WWidget*> unitWidgets, Relation& relation);

    // 添加控件关联关系信号槽
    void addParamRelation(QMap<QString, Relation>* relationMap, const Relation& relation, const QString& relationKey,
                          QMap<int, QMap<QString, WWidget*>> unitDeviceMap);
    // 初始化控件最开始时的关联状态
    void initControlStatus(const Relation& relation);

    int conventWorkMode(SystemWorkMode workMode);
...
#endif  // ParamMacroRelationManager_H
