...
void PlanListWidget::initUI()
{
    ...
    m_impl->m_fastWidget = new SystemLogFastPhotoWidget;
    ...
    m_impl->logMenu->addAction(QString("快照"), this, &PlanListWidget::slotShowFastPhotoLogWidget);
...
void PlanListWidget::slotShowFastPhotoLogWidget()
{
    if (m_impl->m_fastWidget)
    {
        auto currentLogData = m_impl->realTimeLogModel->getCurrentLogData();
        m_impl->m_fastWidget->setFastLogData(currentLogData);
        if (m_impl->m_fastWidget->isHidden())
        {
            m_impl->m_fastWidget->show();
        }
        m_impl->m_fastWidget->raise();
        m_impl->m_fastWidget->activateWindow();
    }
}
