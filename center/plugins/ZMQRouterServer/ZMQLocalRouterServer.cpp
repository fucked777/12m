...
void ZMQLocalRouterServer::run()
{
    ...
    while (m_impl->runningFlag)
    {
        ...
        value >> m_haMsg;
        if (!(m_haMsg.currentMasterIP.isEmpty() || m_impl->masterIP == m_haMsg.currentMasterIP))
        {
            m_impl->masterIP = m_haMsg.currentMasterIP;
            auto communicationPort = ExtendedConfig::clientCommunicationPort();
            m_impl->convertAddr = ZMQHelper::syntheticAddress("tcp", m_haMsg.currentMasterIP, communicationPort);
            /* 重置网络 */
