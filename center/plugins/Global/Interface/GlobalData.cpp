...
void GlobalData::setParamMacroData(const ParamMacroDataList& dataList)
{
    QByteArray data;
    data << dataList;
    RedisHelper::getInstance().setData("ParamMacroDataList", data);
}

bool GlobalData::getParamMacroData(ParamMacroDataMap& dataMap)
{
    QString data;
    if (RedisHelper::getInstance().getData("ParamMacroDataList", data))
    {
        data >> dataMap;
        return true;
    }
...
bool GlobalData::getParamMacroModeData(const QString& taskCode, SystemWorkMode workMode, ParamMacroModeData& paramMacroModeItem)
{
    ...
    if (!getParamMacroData(paramMacroDataMap))
    {
        return false;
    }
    if (!paramMacroDataMap.contains(taskCode))
    {
        return false;
    }
    auto paramMacroData = paramMacroDataMap.value(taskCode);
