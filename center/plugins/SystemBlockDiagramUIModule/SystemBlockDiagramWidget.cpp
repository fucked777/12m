    ...
    , m_impl(new SystemBlockDiagramWidgetImpl)
{
    ...
    bool res = connect(&(m_impl->dataSource), &BlockDiagramDataSource::sg_sendParamData, this, &SystemBlockDiagramWidget::refreshItem);
    /* 这里直接走的信号槽 ,然他走的队列,所以不用加锁 */
    emit m_impl->dataSource.sg_start(m_impl->systemBlockInfo);
...
{
    emit m_impl->dataSource.sg_stop();
