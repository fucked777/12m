...
namespace
{
    ...
    private:
        ...
        public:
            ...
            {
                auto apWidget = new AutorunPolicyWidget(m_context);
                if (m_context)
                {
                    auto ref = m_context.GetServiceReference<AutorunPolicyService>();
                    auto service = m_context.GetService(ref);
                    QObject::connect(apWidget, &AutorunPolicyWidget::sg_modify, service.get(), &AutorunPolicyService::modifyItem);
                    QObject::connect(service.get(), &AutorunPolicyService::sg_modifyACK, apWidget, &AutorunPolicyWidget::modifyACK);
                }
    ...
    public:
        ...
        void Start(BundleContext context)
        {
            ...
            ServiceProperties props;
            props["Type"] = std::string("AutorunPolicyWidget");
            context.RegisterService<IGuiService>(guiService, props);

            registerNetService<AutorunPolicyService>(context, "AutorunPolicyService");
        }
